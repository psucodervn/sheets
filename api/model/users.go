...
type User struct {
	...
	RemindCheckin  null.Bool   `boil:"remind_checkin" json:"remindCheckin,omitempty" toml:"remindCheckin" yaml:"remindCheckin,omitempty"`
...
var UserWhere = struct {
	...
	RemindCheckin  whereHelpernull_Bool
}{
	...
	RemindCheckin:  whereHelpernull_Bool{field: "\"users\".\"remind_checkin\""},
}
...
var (
	userAllColumns            = []string{"id", "created_at", "updated_at", "deleted_at", "name", "email", "hashed_password", "sheet_name", "jira_name", "telegram_id", "remind_checkin"}
	...
	userColumnsWithDefault    = []string{"remind_checkin"}
...
// DaysOffs retrieves all the days_off's DaysOffs with an executor.
func (o *User) DaysOffs(mods ...qm.QueryMod) daysOffQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"days_off\".\"user_id\"=?", o.ID),
	)

	query := DaysOffs(queryMods...)
	queries.SetFrom(query.Query, "\"days_off\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"days_off\".*"})
	}
...
// LoadDaysOffs allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadDaysOffs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`days_off`),
		qm.WhereIn(`days_off.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load days_off")
	}

	var resultSlice []*DaysOff
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice days_off")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on days_off")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for days_off")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.DaysOffs = append(local.R.DaysOffs, foreign)
				if foreign.R == nil {
					foreign.R = &daysOffR{}
				}
				foreign.R.User = local
...
// AddDaysOffsG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.DaysOffs.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddDaysOffsG(ctx context.Context, insert bool, related ...*DaysOff) error {
	return o.AddDaysOffs(ctx, boil.GetContextDB(), insert, related...)
}

// AddDaysOffsP adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.DaysOffs.
// Sets related.R.User appropriately.
// Panics on error.
func (o *User) AddDaysOffsP(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*DaysOff) {
	if err := o.AddDaysOffs(ctx, exec, insert, related...); err != nil {
		panic(boil.WrapErr(err))
	}
}

// AddDaysOffsGP adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.DaysOffs.
// Sets related.R.User appropriately.
// Uses the global database handle and panics on error.
func (o *User) AddDaysOffsGP(ctx context.Context, insert bool, related ...*DaysOff) {
	if err := o.AddDaysOffs(ctx, boil.GetContextDB(), insert, related...); err != nil {
		panic(boil.WrapErr(err))
	}
}

// AddDaysOffs adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.DaysOffs.
// Sets related.R.User appropriately.
func (o *User) AddDaysOffs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*DaysOff) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.DaysOffs = append(o.R.DaysOffs, related...)
	}
