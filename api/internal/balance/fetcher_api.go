...
func (f *ApiFetcher) ListUsers(ctx context.Context) ([]oldmodel.User, error) {
	...
	for i := 0; i < userCount; i++ {
		...
		res = append(res, oldmodel.User{
			Name:    name,
			Balance: oldmodel.Balance{Val: val},
		})
	}
...
func (f *ApiFetcher) ListTransactions(ctx context.Context) ([]oldmodel.Transaction, error) {
...
func toTransaction(raw []interface{}, users []oldmodel.User) (tx *oldmodel.Transaction, err error) {
	defer func() {
		if err != nil {
			return
		}
	...
	// calc senders
	senders := []oldmodel.UserTransaction{
		{Name: toString(raw[2]), Val: toFloat64(raw[1])},
	}
	// calc receivers
	userCount := len(users)
	var receivers []oldmodel.UserTransaction
	for i := 0; i < userCount; i++ {
	...
	tx = &oldmodel.Transaction{
		Description: toString(raw[3]),
		Summary:     toString(raw[3]),
...
func (f *ApiFetcher) ListUserBalances(ctx context.Context) ([]oldmodel.UserBalance, error) {
	rows, err := f.svc.Values.
		Get(f.sheetID, RangeBalance).
		ValueRenderOption("UNFORMATTED_VALUE").
		Do()
	...
	for i := 0; i < userCount; i++ {
		name := rows.Values[1][i].(string)
		val := rows.Values[0][i].(float64)
		res = append(res, oldmodel.UserBalance{
			User:    oldmodel.User{Name: name},
			Balance: oldmodel.Balance{Val: val},
		})
	}
